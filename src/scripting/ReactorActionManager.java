...
public class ReactorActionManager extends AbstractPlayerInteraction {
    ...
    public void dropSingleItem(int itemId) {
        ...
        if (GameConstants.getInventoryType(itemId) != MapleInventoryType.EQUIP) {
            ...
            drop = (Equip) (MapleItemInformationProvider.getInstance().getEquipById(itemId));
        }
        drop.setGMLog("Dropped from reactor " + reactor.getReactorId() + " on map " + getPlayer().getMapId());
...
//                mons.dispelSkill(skil.getSkill());
    ...
    public void doHarvest() { //TODO LEGEND
        ...
        if (getPlayer().getFatigue() >= 0 || getPlayer().getStat().harvestingTool <= 0 || getReactor().getTruePosition().distanceSq(getPlayer().getTruePosition()) > 10000) {
            c.getPlayer().dropMessage(5, "피로도가 부족하여" + pName + "을 할 수 없습니다.");
            return;
        }
