    ...
    public void checkExitAfterGame() {
        for (int i = 0; i < slots; i++) {
            if (exitAfter[i]) {
                ...
                (i == 0 ? getMCOwner() : chrs[i - 1].get()).getClient().getSession().writeAndFlush(PlayerShopPacket.shopErrorMessage(0, i));
                if (i == 0 && getShopType() != 1) {
                    closeShop(false, isAvailable()); //how to return the items?
                } else {
