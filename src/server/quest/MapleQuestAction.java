            ...
            case item:
                ...
                for (QuestItem item : items) {
                    ...
                    if (count < 0) { // remove items
                        ...
                        c.getClient().getSession().writeAndFlush(EffectPacket.showCharmEffect(c, id, 1, true, ""));
                    } else { // add items
                        ...
                        MapleInventoryManipulator.addById(c.getClient(), id, count, "", null, period, "Obtained from quest " + quest.getId() + " on " + FileoutputUtil.CurrentReadable_Date(), false);
                        c.getClient().getSession().writeAndFlush(EffectPacket.showCharmEffect(c, id, 1, true, ""));
                    }
                }
                break;
        ...
        switch (type) {
            case item: {
                ...
                if (c.getInventory(MapleInventoryType.EQUIP).getNumFreeSlot() < eq) {
                    ...
                    return false;
                } else if (c.getInventory(MapleInventoryType.DECORATION).getNumFreeSlot() < cash) {
                    c.dropMessage(1, "Please make space for your DECORATION inventory.");
                    return false;
                }
                return true;
            }
            case money: {
                final long meso = intStore;
