...
public class CashShop implements Serializable {
    ...
    public CashShop(int accountId, int characterId, int jobType) throws SQLException {
        ...
        /*        for (Entry<Long, Item> item : factory.loadItems(false, characterId, null).entrySet()) {
         inventory.add(item.getValue());
         }*/
    }
    ...
    public Item toItem(CashItemInfo cItem, int uniqueid, String gift) {
        ...
        Item ret = null;
        if (GameConstants.getInventoryType(cItem.getId()) == MapleInventoryType.EQUIP || GameConstants.getInventoryType(cItem.getId()) == MapleInventoryType.DECORATION) {
    ...
    public void save(Connection con) throws SQLException {
        Map<MapleInventoryType, List<Item>> itemsWithType = new HashMap<>();
        ...
        for (MapleInventoryType type : types) {
            itemsWithType.put(type, new ArrayList<>());
        }

        for (Item item : inventory) {
            itemsWithType.get(GameConstants.getInventoryType(item.getItemId())).add(item);
        }

        for (Entry<MapleInventoryType, List<Item>> iter : itemsWithType.entrySet()) {
            if (con != null) {
                factory.saveItems(iter.getValue(), con, accountId, iter.getKey(), false);
            } else {
                factory.saveItems(iter.getValue(), accountId, iter.getKey(), false);
            }
        }
    }
}
