...
function action (mode, type, selection) {
    ...
    if (status == 0) {
      ...
      //  "#L1##b" + ConvertNumber(keep) + "메소를 사용하여 파괴, 미끄러짐을 방지 하겠습니다.#l\r\n" +
        ...
        say += "#fs 11#";
        say += "강화 : #b0강 -> 10강#k\r\n";
        say += "강화 성공 시 #b올스탯 + 48 공마 + 27#k 증가\r\n";
        ...
        if (cm.getInventory(1).getItem(slot) != null) {
            ...
            if (rand >= 0 && rand <= items[getAddEnhance(item) + 1][1]) {
                item.setStr(item.getStr() + 48);
                item.setDex(item.getDex() + 48);
                item.setInt(item.getInt() + 48);
                item.setLuk(item.getLuk() + 48);
                item.setWatk(item.getWatk() + 27);
                item.setMatk(item.getMatk() + 27);
