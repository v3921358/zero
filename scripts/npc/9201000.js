...
//        if (cm.getPlayerStat("GENDER") == 0) {
            ...
            if (hasEngageRing) {
                ...
                task = 1;
                cm.sendNext("약혼 반지 제작을 도와드리도록 하지요.");
            } else if (cm.getQuestStatus(1100) == 1 || cm.getQuestStatus(1100) == 2) {
                if (cm.getPlayerStat("LVL") < 200) {
                    cm.sendOk("음.. 당신은 사랑하는 사람을 지키기엔 아직 너무 약한 것 같군요. 레벨 200 이 되시면 다시 찾아오세요.");
                    cm.dispose();
                    return;
                }
    ...
    //        cm.dispose();
      //  }
        ...
        if (task == 1) {
            ...
            cm.sendOk("당신의 약혼을 축복하기 위해 무료로 드리겠습니다!\r\n다시 저에게 대화를 걸어주세요.");
