...
function action(mode, type, selection) {
    ...
    setting = [
        ["Hard_Lotus", count, 350060700, 230],
        ["Normal_Lotus", count, 350060400, 220],
        ["Extreme_Lotus", 1, 350060700, 290],
    ]
    name = ["하드", "노멀", "익스트림"]
    if (mode == -1 || mode == 0) {
        cm.dispose();
    ...
    if (status == 0) {
        ...
        talk += "#L1#블랙헤븐 코어(노멀 모드)로 이동 한다.(레벨 220이상)#l\r\n";
      ...
      //  talk += "#L2#블랙헤븐 코어(익스트림 모드)로 이동 한다.(레벨 190이상)#l\r\n";
        ...
        if (!cm.isBossAvailable(setting[st][0], setting[st][1])) {
            ...
            talk += "#k#n님이 오늘 입장했습니다. 스우 " + name[st] + "모드는 하루에 " + setting[st][1] + "번만 도전하실 수 있습니다.";
