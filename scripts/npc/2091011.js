...
function action(mode, type, selection) {
	...
	s2 = selection;
	if (s1 == 1) {
		cm.sendNextPrev("사부님이 계신 63층을 제외한 각 층엔 월드의 보스들이 그곳을 지키고 있지. 자세한 사정은 나도 몰라. 사부님만이 아실 뿐.");
	} else if (s2 == 0) {
	...
	if (s1 == 1) {
		cm.sendNextPrev("40층부터는 10층 단위로 네임드 보스들이 등장해. 여기선 #r15초#k마다 포션을 사용할 수 있어. 어줍잖은 실력으로 만나면 고생 좀 할 거야.");
	}
    } else if (status == 7) {
	if (s1 == 1) {
	...
	if (s1 == 1) {
		cm.dispose();
		cm.sendNextPrev("게다가 사부님의 결계로 인해서 네가 발휘하던 힘의 10분의 1밖에 쓰지 못하니까 들어가서 당황하지마.\r\n알아들었으면 어서 들어가 보라고. 몸이 근질근질하지 않아?");
	}
    }
}