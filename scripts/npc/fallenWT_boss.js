...
function action(mode, type, selection) {
    ...
    if (cm.getPlayer().getBossTier() >= 5) {
        count = 3
    }
    setting = [
        ["Normal_Demian", count, 350160100, 220],
        ["Hard_Demian", count, 350160200, 230],
        ["Extreme_Demian", 1, 350160200, 290]
    ]
    name = ["노멀", "하드", "익스트림"];
    if (mode == -1 || mode == 0) {
        cm.dispose();
    ...
    if (status == 0) {
        ...
        //if (cm.getPlayer().getGMLevel() > 5) {
            //talk+= "#L2#타락한 세계수 정상(#b익스트림 모드#k)으로 이동 한다.(레벨 190이상)#l\r\n";
        //}
        ...
        if (cm.getParty() == null) {
            ...
            cm.dispose();
            return;
        } else if (cm.getPlayerCount(setting[st][2]) >= 1 || cm.getPlayerCount(Number(setting[st][2]) + 40) >= 1) {
        ...
        if (!cm.isBossAvailable(setting[st][0], setting[st][1])) {
            ...
            talk += "#k#n님이 오늘 입장했습니다. 데미안 " + name[st] + "모드는 하루에 " + setting[st][1] + "번만 도전하실 수 있습니다.";
