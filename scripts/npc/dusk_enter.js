...
function action(mode, type, selection) {
    ...
    setting = [
        ["Normal_Dusk", count, 450009400, 240],
        ["Chaos_Dusk", count, 450009450, 260]
    ]
    name = ["노멀"];
    if (mode == -1 || mode == 0) {
        cm.dispose();
    ...
    if (status == 0) {
        talk = "검은 마법사의 사념으로 이루어진 거대 괴수 더스크를 내버려 두어선 안된다.\r\n\r\n#L0##b공허의 눈(노멀 모드)으로 이동한다.#r(레벨 240이상)#k#l\r\n#l#L1##b공허의 눈(카오스 모드)으로 이동한다.#r(레벨 260이상)#k#l\r\n#l";
        ...
        if (!cm.isBossAvailable(setting[st][0], setting[st][1])) {
            ...
            talk += "#k#n님이 오늘 입장했습니다. 더스크 " + name[st] + "모드는 하루에 " + setting[st][1] + "번만 도전하실 수 있습니다.";
