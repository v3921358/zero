function enter(pi) {
    pi.getClient().getSession().writeAndFlush(Packages.tools.packet.MaplePacket.SetIngameDirectionMode(true, true, true, false));
    ...
    Packages.server.Timer.MapTimer.getInstance().schedule(function () {
        pi.getClient().getSession().writeAndFlush(Packages.tools.packet.MaplePacket.SetIngameDirectionMode(false, true, false, false));
    }, 2000)
}
